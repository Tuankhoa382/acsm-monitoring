...
# ==============================================================================
# 2. HÀM HỖ TRỢ & BỘ LỌC KALMAN (ĐÃ CLEAN SẠCH)
# ==============================================================================
def auto_fix_cookie_file():
    """Tự động sửa lỗi định dạng file cookie nếu có."""
...
def play_laptop_sound():
    """Phát âm thanh hệ thống (CHỈ TRÊN WINDOWS)"""
    try:
        import winsound
...
def play_web_sound_script():
    """Chèn JavaScript để cưỡng bức trình duyệt phát âm thanh."""
    st.markdown(
        f"""
...
class SimpleKalmanFilter:
    """Class đã sửa lỗi Trailing Semicolon"""
...
class WorkerTracker:
    ...
    def get_smoothed_position(self):
        # Hàm này đã được khôi phục và đảm bảo đúng cấu trúc
...
# --- CUSTOM CSS CHO FULL SCREEN & THẨM MỸ ---
st.markdown("""
    <style>
    /* Fix Full Screen */
    ...
    .stAlert-danger { background-color: #c0392b !important; color: white; }
    .stAlert-warning { background-color: #f39c12 !important; color: black; }
    .stAlert-success { background-color: #1abc9c !important; color: white; }
    </style>
    """, unsafe_allow_html=True)
...
while True:
    with placeholder.container():
        try:
            service = Service(cookies_file=COOKIES_FILE, authenticating_account='nguyendangkhoa420614@gmail.com')
                        ...
                        if age_min > 5:
                            ...
                            if kf_dist > SAFE_RADIUS:
                                ...
                                if dist_moved < 5.0:
                                    ...
                                    st.markdown(
                                        '<div class="stAlert stAlert-success">✅ ĐANG DI CHUYỂN TRONG VÙNG</div>',
                                        unsafe_allow_html=True)
